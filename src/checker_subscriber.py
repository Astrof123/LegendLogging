with open("tmp/subscriber.log", "r") as f:
    logs = f.readlines()
    phrases = ["Подписчик получил id", "Подписчик запустил приложение с id",
               "Подключение к брокеру", "Идет прослушка по теме",
               "Полученное сообщение", "Прослушка закончена"]
    ...
    def findError(user_id):
        ...
        for line in logs:
            ...
            if flag and user_id in line:
                if phrases[k] == "Полученное сообщение":
